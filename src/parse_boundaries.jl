...
struct LowerBoundary <: AbstractTruncatingBoundary
    ...
    function LowerBoundary(u, t, x, order, eq, v)
        ...
        allx̄ = Set(filter(!isempty,
            map(u -> filter(x -> t === nothing || !isequal(x, t), arguments(u)), depvars)))
...
function has_interfaces(bmps)
    any(b -> b isa InterfaceBoundary,
        reduce(vcat, reduce(vcat, collect.(values.(collect(values(bmps)))))))
end
